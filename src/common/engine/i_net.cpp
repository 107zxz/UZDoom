...
static TArray<uint64_t> BannedConnections               = {};
...
void CloseNetwork()
{
	...
	if (SteamAPI_IsSteamRunning())
	{
		...
		I_NetLog("Closed Steam P2P sessions for all connected peers");
	}
...
static int FindClient(uint64_t steamID)
{
	int i = 0;
	for (; i < MaxClients; ++i)
	{
		...
		if (steamID == Connected[i].SteamID)
		{
			break;
		}
	}
...
static void GetPacket(uint64_t *from = nullptr)
{
	// size_t   fromSize = sizeof(fromSteamID);

	// int msgSize = recvfrom(MySocket, (char *)TransmitBuffer, MaxTransmitSize, 0, (sockaddr *)&fromAddress,
	// &fromSize);

	CSteamID fromSteamID;
	uint32_t msgSize;
	...
	else if (msgSize > 0)
	{
		...
		else if (client == -1 && bGameStarted)
		{
			...
			NetBufferLength = 2u;
			SendPacket(fromSteamID.ConvertToUint64());
			msgSize = 0;
		}
		else
		{
			...
			const uint32_t crc =
				(TransmitBuffer[0] << 24) | (TransmitBuffer[1] << 16) | (TransmitBuffer[2] << 8) | TransmitBuffer[3];
			if (check != crc)
			{
	...
	if (from != nullptr)
		*from = fromSteamID.ConvertToUint64();
}
...
void I_NetCmd(ENetCommand cmd)
{
	...
	{
		if (RemoteClient >= 0)
			SendPacket(Connected[RemoteClient].SteamID);
	}
	else if (cmd == CMD_GET)
	{
		GetPacket();
...
static bool GetConnection(uint64_t &from)
{
	GetPacket(&from);
	return NetBufferLength > 0;
}

static void RejectConnection(uint64_t to, ENetConnectType reason)
{
	NetBuffer[0]    = NCMD_SETUP;
	NetBuffer[1]    = reason;
	NetBufferLength = 2u;
...
static void SendVerificationError(uint64_t toSteamID, const FVerificationError &error)
{
	NetBuffer[0] = NCMD_SETUP;
	...
	SendPacket(toSteamID);
}

static void AddClientConnection(uint64_t fromSteamID, int client)
{
	Connected[client].Status  = CSTAT_CONNECTING;
	Connected[client].SteamID = fromSteamID;
...
static void RemoveClientConnection(int client)
{
	...
	for (int i = 1; i < MaxClients; ++i)
	{
		...
		for (int i = 0; i < 4; ++i)
			SendPacket(Connected[i].SteamID);
	}
}
...
static bool Host_CheckForConnections(void *connected)
{
	...
	for (auto client : toBoot)
	{
		...
		uint64_t booted = Connected[client].SteamID;
			...
			for (int i = 0; i < MaxClients; ++i)
			{
				...
				if (!ClientGotAck(client, i))
				{
					if (Connected[i].Status >= CSTAT_WAITING)
					{
						...
						if (i > 0)
						{
							memcpy(&NetBuffer[NetBufferLength], &Connected[i].SteamID, addrSize);
	...
	if (consoleplayer == 0)
	{
		for (int client = 1; client < MaxClients; ++client)
		{
			if (Connected[client].Status != CSTAT_NONE)
				SendPacket(Connected[client].SteamID);
		}
	}
	else
	{
...
static bool HostGame(int arg)
{
	...
	// Not in start network cos its used for join
...
static FString ReadVerificationError(TArrayView<uint8_t> stream)
{
	...
	{
		return FStringf("Engine mismatch: host expected %d.%d.%d, got %d.%d.%d", stream[1], stream[2], stream[3],
		...
		else if (NetBuffer[1] == PRE_USER_INFO)
		{
			...
			if (!ClientGotAck(consoleplayer, c))
			{
				...
				if (c > 0)
				{
					...
					memcpy(&Connected[c].SteamID, &NetBuffer[byte], addrSize);
					byte += addrSize;
				}
				else
				{
...
{
	if (arg >= Args->NumArgs() || Args->GetArg(arg)[0] == '-' || Args->GetArg(arg)[0] == '+')
	{
	...
	// BuildAddress(Connected[0].SteamID, Args->GetArg(arg));

	// Set the host's steam id to the passed join arg
	Connected[0].SteamID = std::stoull(Args->GetArg(arg));
